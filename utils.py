...
def load_data_up(x_path, y_path, u_path, p_path):
    x = []
    y = []
    u = []
    p = []
    ...
    for line in open(y_path, 'rb'):
        label = line.strip()
        y.append(int(label))
    for line in open(u_path, 'rb'):
        label = line.strip()
        u.append(int(label))
    for line in open(p_path, 'rb'):
        label = line.strip()
        p.append(int(label))

    return np.array(x), np.array(y), np.array(u), np.array(p)
